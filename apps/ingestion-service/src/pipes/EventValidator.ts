...
export function createEventValidatorPipe(
  givendomain: EventDomain = 'IdentityEvents',
) {
  @Injectable()
  class EventValidatorPipe implements PipeTransform {
    transform(data: any) {
      try {
        ...
        const eventSchema = domainRegistry[
          eventName as keyof typeof domainRegistry
        ] as z.ZodObject<any>;

        if (!eventSchema) {
          throw new BadRequestException(
            'No shcema found for the given event Name',
          );
        }
